# Post: Der Kunde merkt es zuerst

Datum: 2026-09-03 (Donnerstag), 07:30
Format: Story plus Post-it-Bild mit der Lösung, kleiner Pitch in der Mitte
Bild: skizzen/bild-postit.png (gelbes Post-it, "Jede Anfrage wird sofort bestätigt", Unterzeile "Die Offerte darf dauern. Das Schweigen nicht.")
Alternativbild: skizzen/bild-ruf.png (Zwei-Spalten-Skizze innen gegen aussen)
Content-Säule: Schmerz-Story mit kleinem Pitch in der Mitte
Thema: Ruf von aussen, komplett unbespielt (bisher alles Innensicht)
Gewerk: Sanitär (Badsanierung). Nächste Posts rotieren: Gipser, Maler, Elektro, Schreiner.

## Post

Dein Ruf als Sanitär wird gerade beschädigt. Nur nicht auf der Baustelle.

Von innen fühlt sich dein Betrieb normal an. Zwanzig Mitarbeiter, die Baustellen laufen, die Arbeit stimmt. Aber der Kunde, der vor fünf Tagen wegen der Badsanierung angefragt hat, erlebt einen anderen Betrieb. Er sieht deinen Schreibtisch nicht, den Stapel, den Termin, der länger ging. Er sieht nur, dass nichts kommt.

Am zweiten Tag denkt er, es sei halt viel los. Am vierten zieht er den einzigen möglichen Schluss: So wie die mit Anfragen umgehen, arbeiten die wahrscheinlich auch auf der Baustelle. Das stimmt nicht, aber es ist alles, was er von dir gesehen hat. Am fünften ruft er beim nächsten Sanitär an.

Und du merkst nichts davon. Keine Absage, kein böses Wort, einfach eine Anfrage weniger.

Zwanzig Jahre Baustelle bauen deinen Ruf auf. Fünf stille Tage im Posteingang bauen ihn ab.

Dabei würde eine Bestätigung reichen, die von selbst rausgeht. Sie beantwortet nichts, sie sagt dem Kunden nur, dass er nicht ins Leere geschrieben hat. Das ist der kleinste Baustein, mit dem ich in Betrieben anfange.

Wie alt ist die älteste Anfrage, die bei dir noch auf eine Antwort wartet?

## Alternative Hooks

1. Seit fünf Tagen wartet eine Badsanierung auf deine Antwort.
2. Zwanzig Mitarbeiter im Sanitär, und du verlierst Kunden, die nie absagen.

Beide nennen das Gewerk ebenfalls in Zeile eins.

Experten-Runde 02.09. eingearbeitet: Zeitlinie gedreht (Urteil vor Wechsel), Singular-Hook, Anfrage-Referenz repariert, Arbeit statt Qualität.

## Geparkt: Giveaway-Variante

Der gleiche Post mit dem Schluss "Dagegen gibt es fünf einfache Gegenmittel, keines braucht neue Software. Ich habe sie als kurzes PDF aufgeschrieben. Soll ich es dir schicken?" liegt bereit. Einsetzen, sobald die Followerbasis gross genug ist, dass unter dem Post sichtbar Kommentare zusammenkommen. Bei drei Ja-Kommentaren wirkt ein Giveaway leerer als gar keins. Bis dahin arbeitet das PDF im Profil unter "Im Fokus" und als Anhang in Direktnachrichten nach dem Erstkontakt.

## Warum dieses Thema ganz neu ist

Alle bisherigen Posts erzählten die Innensicht (dein Tisch, deine Zeit, dein Engpass). Dieser erzählt zum ersten Mal die Aussensicht: was der Kunde in den fünf stillen Tagen denkt. Und die 250 vom Naturgesetz-Post bestätigen das Muster: erzählte Dritte (Gipser-Szenen) deckeln bei 250 bis 313, volle Du-Posts holen 662 plus. Dieser Post bleibt durchgehend beim Du.

## Ablauf am Donnerstag

07:30 posten, Bild `skizzen/bild-postit.png` als Einzelbild anhängen. Kein Link im Post und keiner im ersten Kommentar, beides kostet Reichweite.

Bis 09:00 erreichbar bleiben, das ist das Fenster, in dem rund 70 Prozent der Reichweite entschieden werden. Jeden Kommentar sofort mit einem echten Satz beantworten, nie mit Danke. Wer eine Zahl nennt ("bei mir liegt eine von letzter Woche"), bekommt eine Rückfrage zu genau dieser Anfrage. Daraus wird der Gesprächseinstieg, und erst in der Direktnachricht kommt das PDF ins Spiel.

Danach auf drei bis fünf fremden Handwerker-Posts kommentieren.

## Offener Test

Erster Post mit Einzelbild. Die Recherche vom 02.09. sagt, Einzelbilder liegen rund 30 Prozent hinter reinem Text. Gegenprobe: Landet dieser Post unter dem Text-Median von rund 176, läuft der nächste wieder blank.

## Ergebnis

Impressions:
Reaktionen:
Kommentare:
Erkenntnis:
